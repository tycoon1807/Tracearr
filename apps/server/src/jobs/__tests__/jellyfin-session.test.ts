/**
 * Jellyfin session mapping tests
 *
 * Tests for Jellyfin-specific helpers:
 * - isPrivateIP: Filter local network IPs to avoid GeoIP lookup failures
 * - parseJellyfinClient: Parse client strings into platform/device info
 */

import { describe, it, expect } from 'vitest';
import { isPrivateIP, parseJellyfinClient } from '../poller.js';
...
describe('parseJellyfinClient', () => {
  ...
  describe('Android devices', () => {
    ...
    it('should parse Shield clients as Android TV', () => {
      // "Jellyfin for Shield" doesn't contain "android" but contains "shield"
  ...
  describe('deviceType parameter', () => {
    ...
    it('should prefer deviceType over parsing when deviceType is meaningful', () => {
      // Even if client contains "iOS", deviceType takes precedence
...
// NOTE: Bitrate calculation tests were removed because they tested JavaScript
// language behavior (|| operator, Math.round) rather than actual production code.
// The formatQualityString function is now exported from poller.ts and tested
// in poller.test.ts which imports and tests the actual function.
